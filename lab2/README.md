...
**Основные задачи:**
1. Принять на вход строку функции f(x) (липшицевой), границы [a, b] и точность eps.
2. Реализовать метод Пиявского для поиска глобального минимума.
3. Автоматически оценить константу Липшица L (при отсутствии ручного ввода).
4. Визуализировать процесс — график функции, нижнюю оценку (ломаную), вычисленные точки, найденный минимум.
5. Вывести числовые результаты: аргумент минимума, значение функции, количество итераций и затраченное время.
6. Сформировать PDF-отчёт с результатами и графиками.
...
```
f(x) = -20*exp(-0.2*sqrt(x**2)) - exp(0.5*(cos(2*pi*x) + 1)) + 20 + e
```
...
## Визуализация и PDF-отчет

Программа автоматически создает PDF-отчет `piyavskii_report.pdf` со следующей структурой:

### Структура PDF-отчета:

**Страница 1:** График тестовой функции `(x-2)² + 5*sin(π*x)`  
**Страница 2:** График функции Растригина  
**Страница 3:** График функции Экли (Ackley)  
**Страница 4:** Текстовая сводка с числовыми результатами всех тестов

### Что отображается на каждом графике:

1. **Синяя сплошная линия** — исходная функция f(x), построенная на 2000 точках
2. **Пунктирная линия** — итоговая ломаная нижней оценки g(x) = max_i g_i(x), где g_i(x) = f(x_i) - L|x - x_i|
3. **Красные точки** — все точки, где метод вычислял значение функции (пробные точки алгоритма)
4. **Звёздочка (★)** — найденный глобальный минимум с координатами (x_min, f(x_min))

### Текстовая сводка (страница 4):

Содержит числовые результаты для всех трех тестовых функций:
- Формула функции
- Отрезок поиска [a, b]
...
- Количество итераций
- Затраченное время
- Найденный минимум: x_min и f(x_min)
