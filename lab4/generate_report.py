...
2. **Применение сценария:**
   $$s_{{t+1}} = \\omega_t \\odot s^{{\\prime}}_t$$
   
   где $\\odot$ - поэлементное умножение (применение мультипликаторов):
   $$s_{{t+1}} = (s^{{\\prime}}_{{CB1}} \\cdot \\omega_{{CB1}}, s^{{\\prime}}_{{CB2}} \\cdot \\omega_{{CB2}}, s^{{\\prime}}_{{Dep}} \\cdot \\omega_{{Dep}}, c^{{\\prime}}_t)$$
   
   где $s^{{\\prime}}_{{CB1}}, s^{{\\prime}}_{{CB2}}, s^{{\\prime}}_{{Dep}}$ - стоимости активов после действия, $\\omega_{{CB1}}, \\omega_{{CB2}}, \\omega_{{Dep}}$ - мультипликаторы сценария, $c^{{\\prime}}_t$ - касса после действия.
...
| Метрика | Значение |
|---------|----------|
| Ожидание ($\mu$) | {mc_results.get('mean', 0):.2f} д.е. |
| Стд.откл ($\sigma$) | {mc_results.get('std', 0):.2f} д.е. |
