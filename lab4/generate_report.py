...
1. **Применение действия:**
   ...
   $$C(u_t) = \\sum_{{x \\in \\{{CB1, CB2, Dep\\}}}} \\text{{eff\\_cost}}(\\Delta_x, \\text{{comm}}_x)$$

2. **Применение сценария:**
   $$s_{{t+1}} = \\omega_t \\odot s'_t$$
   
   где $\\odot$ - поэлементное умножение (применение мультипликаторов):
   $$s_{{t+1}} = (s'_{{CB1}} \\cdot \\omega_{{CB1}}, s'_{{CB2}} \\cdot \\omega_{{CB2}}, s'_{{Dep}} \\cdot \\omega_{{Dep}}, s'_{{cash}})$$

#### Расчет эффективной стоимости

При покупке ($\\Delta > 0$):
$$\\text{{eff\\_cost}}(\\Delta, c) = \\Delta \\cdot (1 + c)$$
...
$$\\sum_{{x}} \\text{{eff\\_cost}}(\\Delta_x, \\text{{comm}}_x) \\leq \\text{{cash}}_t$$
