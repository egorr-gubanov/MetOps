...
6. [Дополнительные возможности](#дополнительные-возможности-для-получения-дополнительных-баллов)
...
├── dual_solver.py              # Решение двойственной задачи (НОВОЕ)
├── integer_solver.py           # Решение задач ЦЛП методом ветвей и границ (НОВОЕ)
└── test_problems/              # Директория с тестовыми задачами
    ├── problem01.txt
    ├── problem02.txt
    ├── ... (до problem20.txt)
    ├── dual_example.txt        # Пример для двойственной задачи (НОВОЕ)
    ├── integer_example.txt     # Пример для целочисленного программирования (НОВОЕ)
    └── integer_example2.txt    # Еще один пример ЦЛП (НОВОЕ)
```
...
## Дополнительные возможности (для получения дополнительных баллов)

### 1. Решение двойственной задачи

Программа может автоматически построить и решить двойственную задачу для любой исходной задачи линейного программирования.
...
1. Решает исходную (примальную) задачу
2. Строит двойственную задачу по правилам двойственности:
   - Инвертирует тип целевой функции (min ↔ max)
   - Коэффициенты ЦФ двойственной = правые части ограничений исходной
   - Правые части ограничений двойственной = коэффициенты ЦФ исходной
   - Матрица ограничений двойственной = транспонированная матрица исходной
3. Решает двойственную задачу
4. Проверяет теорему двойственности: Z* = W*

**Пример вывода:**
```
================================================================================
РЕШЕНИЕ ИСХОДНОЙ (ПРИМАЛЬНОЙ) ЗАДАЧИ
================================================================================
...
...
Maximize: W = 9.00·y1 + 7.00·y2 + 5.00·y3

При ограничениях:
  (1) 2.00·y1 + 1.00·y2 + 0.00·y3 <= 4.00
  (2) 1.00·y1 + 1.00·y2 + 0.00·y3 <= 1.00
  ...
...
Значение целевой функции исходной задачи: Z* = 7.000000
Значение целевой функции двойственной задачи: W* = 7.000000
Разница: |Z* - W*| = 0.000000
✓ Теорема двойственности подтверждена: Z* = W*
```
...
Программа реализует метод ветвей и границ (Branch and Bound) для решения задач, где переменные должны принимать целочисленные значения.
...
# Только указанные переменные целочисленные (1-based)
python integer_solver.py test_problems/integer_example.txt 1 2
```

**Алгоритм:**
1. Решает релаксацию задачи (без требования целочисленности)
2. Если решение целочисленное - готово
3. Если нет - выбирает переменную с наибольшей дробной частью
4. Создает две подзадачи:
   - Одна с ограничением x ≤ ⌊x*⌋
   - Другая с ограничением x ≥ ⌈x*⌉
5. Рекурсивно решает подзадачи
6. Отсекает подзадачи, которые не могут дать лучшее решение (bound)
...
Целочисленные переменные: [1, 2]
================================================================================

✓ Найдено целочисленное решение (узел 3):
  Z = 14.000000
