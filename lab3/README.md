...
$\lambda_{2}^{*} = -\frac{11}{8} = -1.375$  
$\lambda_{1}^{*} = 5 + \lambda_{2}^{*} = 5 - 1.375 = \frac{29}{8} = 3.625$

### 4. Значение целевой функции

$$f(x^{\ast}, y^{\ast}) = 4 \cdot 0.75^{2} + 2 \cdot 0.75 \cdot (-1.25) + 5 \cdot (-1.25)^{2} - 4 \cdot 0.75 + 6 \cdot (-1.25)$$
...
4. **Множители Лагранжа:** Найденные значения $\lambda_{1}^{*} = 3.625$ и $\lambda_{2}^{*} = -1.375$ показывают "цену" каждого ограничения в точке оптимума.
